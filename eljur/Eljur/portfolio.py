from eljur.Eljur.errors import _fullCheck
...
class Portfolio:
    ...
    def reportCard(self, subdomain, session, user_id=None, quarter="I"):
        """
        ...
        :return: Словарь с ошибкой или ответом (отсутствие оценок или словарь с оценками)  /u.{user_id}     // dict
        """

        url = f"https://{subdomain}.eljur.ru/journal-student-grades-action/sp.{quarter}+четверть"
        ...
        for subject in subjects:
            ...
            for score in soup.find_all("div", class_=["cell blue", "cell"], attrs={"name": subject.contents[0]}):
                if "mark_date" in score.attrs:
                    if score.attrs["id"] != "N":
                        scores.append(
                            {score.attrs["mark_date"], score.contents[1].contents[0]})
